---
title: Termos de Uso — ViaCiclo
---

# Termos de Uso

**Última atualização:** 20 de abril de 2026

Ao instalar, acessar ou usar o aplicativo **ViaCiclo**, você concorda com estes Termos de Uso. Se você não concorda, não use o aplicativo.

---

## 1. Sobre o ViaCiclo

O **ViaCiclo** é um aplicativo gratuito para smartphones Android que oferece:

- Visualização de ciclovias e ciclofaixas em cidades brasileiras
- Cálculo de rotas seguras para bicicleta
- Rastreamento de pedaladas com GPS
- Compartilhamento de rotas entre ciclistas
- Ranking e gamificação

**Responsável:** Vinicius Brasil Sanchez (MEI) — CNPJ **65.139.333/0001-41**
**Contato:** viaciclo.app@gmail.com

---

## 2. Cadastro e conta

Para usar o aplicativo, é necessário fazer login com uma **conta Google** válida. Você é responsável por manter a confidencialidade da sua conta.

Você pode excluir sua conta a qualquer momento enviando um pedido para o email de contato.

---

## 3. Uso aceitável

Você concorda em **não**:

- Usar o app para fins ilícitos
- Compartilhar rotas com conteúdo ofensivo, ilegal ou que viole direitos de terceiros
- Tentar acessar dados de outros usuários sem autorização
- Fazer engenharia reversa, descompilar ou modificar o app
- Usar scripts, bots ou automação para inflar pontos, curtidas ou estatísticas

O descumprimento pode resultar em suspensão ou exclusão da conta.

---

## 4. Conteúdo gerado pelo usuário

Ao compartilhar uma rota na aba Social, você:

- Declara que essa rota foi pedalada por você
- Concede ao ViaCiclo licença não exclusiva, gratuita e mundial para exibir a rota dentro do aplicativo
- Permite que outros usuários usem essa rota como referência para pedaladas próprias
- Pode deletar a rota a qualquer momento

Você mantém todos os direitos sobre o conteúdo que cria.

---

## 5. Limitação de responsabilidade

### 5.1. Segurança no trânsito

O ViaCiclo **é uma ferramenta de apoio** e **não substitui** bom senso, atenção ao trânsito, respeito às leis de circulação e uso de equipamentos de segurança.

**Você é o único responsável por**:

- Sua segurança ao pedalar
- Seguir as leis de trânsito e sinalizações
- Usar capacete, luzes e demais equipamentos de proteção
- Avaliar as condições reais da via (obras, buracos, trânsito, clima) antes e durante a pedalada

Rotas calculadas pelo app podem conter imprecisões, vias em obras ou trechos temporariamente interditados. Sempre verifique as condições reais antes de iniciar.

### 5.2. Dados de GPS

A precisão da localização depende do seu dispositivo e das condições do sinal. O ViaCiclo não se responsabiliza por erros de cálculo de distância, velocidade ou rota decorrentes de falhas do GPS.

### 5.3. Disponibilidade do serviço

O app é fornecido **"como está"** (as is). Não garantimos disponibilidade ininterrupta, ausência de erros ou que o serviço atenderá a todas as suas expectativas. Manutenções, atualizações ou falhas de serviços de terceiros (Google, Supabase, OpenStreetMap) podem causar indisponibilidade temporária.

### 5.4. Limites

Na máxima extensão permitida pela lei, o ViaCiclo e seu responsável **não respondem por danos indiretos, incidentais ou consequenciais** decorrentes do uso do aplicativo, incluindo acidentes de trânsito, lesões pessoais, danos materiais ou perda de dados.

---

## 6. Propriedade intelectual

- O **código-fonte** do ViaCiclo é disponibilizado publicamente em [github.com/sanchezVB/viaciclo_app](https://github.com/sanchezVB/viaciclo_app).
- **Marca, nome e logo "ViaCiclo"** pertencem ao responsável pelo app.
- **Dados de ciclovias** são obtidos do OpenStreetMap sob licença [ODbL](https://www.openstreetmap.org/copyright).
- **Mapas** são fornecidos pelo [OpenStreetMap](https://www.openstreetmap.org/).
- **Cálculo de rotas** é fornecido pelo [OpenRouteService](https://openrouteservice.org/).

---

## 7. Privacidade

O tratamento dos seus dados pessoais é regido pela nossa [Política de Privacidade](privacy.html), que faz parte integrante destes Termos.

---

## 8. Alterações nos Termos

Podemos alterar estes Termos a qualquer momento. Alterações significativas serão comunicadas por email ou notificação no app com antecedência mínima de 15 dias. O uso continuado do app após a alteração significa aceitação dos novos termos.

---

## 9. Rescisão

Você pode parar de usar o app a qualquer momento, bastando desinstalá-lo. Podemos encerrar ou suspender seu acesso caso você descumpra estes Termos.

---

## 10. Lei aplicável e foro

Estes Termos são regidos pelas leis da República Federativa do Brasil. Fica eleito o foro da comarca de São Paulo/SP para dirimir eventuais conflitos.

---

*Documento vigente a partir de 20 de abril de 2026.*
